...
class FleetWarehouse:
    ...
    def list_sessions(self,
        ...
        return [
            SessionMetadata(**json.loads(row["metadata_json"]))
            for row in rows
        ]
    ...
    def get_trend(self,
                  ...
                  gpu_name: Optional[str] = None,
                  since_days: int = 30,
                  limit: int = 1000) -> List[TrendPoint]:
        """
    ...
    def get_metric_stats(self,
                         ...
                         model_name: Optional[str] = None,
                         backend: Optional[str] = None,
                         since_days: int = 7) -> Dict[str, float]:
        """
    ...
    def compare_backends(self,
                         metric_name: str,
                         model_name: str,
                         since_days: int = 7) -> Dict[str, Dict[str, float]]:
        """
    ...
    def compare_gpus(self,
                     ...
                     model_name: str,
                     backend: str,
                     since_days: int = 30) -> Dict[str, Dict[str, float]]:
        """
    ...
    def detect_regression(self,
                          ...
                          baseline_days: int = 7,
                          current_days: int = 1,
                          threshold_pct: float = 10.0) -> Optional[Dict[str, Any]]:
        """
        ...
        with self._get_connection() as conn:
            baseline_row = conn.execute(query, (
                metric_name, model_name, baseline_cutoff, current_cutoff
            )).fetchone()
            
            current_row = conn.execute(query, (
                metric_name, model_name, current_cutoff, now
            )).fetchone()
