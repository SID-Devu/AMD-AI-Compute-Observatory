...
class FleetAggregator:
    ...
    def _compare_nodes(
        self, results: List[JobResult], metric_name: str, extractor: callable
    ) -> NodeComparison:
    ...
    def compare_gpus(self, workload_id: str, gpu_node_mapping: Dict[str, str]) -> GPUComparison:
        """
        ...
        if comparison.gpu_values:
            ...
            for gpu_type, summary in comparison.gpu_values.items():
                comparison.normalized_values[gpu_type] = (
                    summary.mean / fastest_mean if fastest_mean > 0 else 0
                )
