...
class OpKernelMapper:
    ...
    KERNEL_PATTERNS = {
        ...
        "MatMul": [
            r"gemm", r"rocblas_gemm", r"hipblas_gemm", r"Cijk", r"matmul",
            r"sgemm", r"dgemm", r"hgemm", r"rocblas_internal",
        ],
        "Conv": [
            r"Conv", r"miopenConv", r"ImplicitGemm", r"winograd",
            r"ConvolutionBackward", r"col2im", r"im2col",
        ],
        "BatchNormalization": [
            r"batchnorm", r"BatchNorm", r"bn_", r"miopenBatchNorm",
        ],
        "LayerNormalization": [
            r"layernorm", r"LayerNorm", r"ln_", r"layer_norm",
        ],
        "Softmax": [
            r"softmax", r"Softmax", r"exp_", r"reduce_sum",
        ],
        "Attention": [
            r"attention", r"Attention", r"flash_attn", r"fused_attention",
            r"multihead", r"qkv", r"scaled_dot",
        ],
    ...
    def _group_kernels(self) -> None:
        ...
        for group_id, (group_name, kernels) in enumerate(groups.items()):
            ...
            self.kernel_groups.append(KernelGroup(
                group_id=group_id,
    ...
    def get_hotspot_ops(self, top_n: int = 10) -> List[Dict[str, Any]]:
        ...
        sorted_attrs = sorted(
            self.attributions,
            key=lambda a: a.attributed_time_ms,
            reverse=True
        )
    ...
    def get_kernel_fragmentation_score(self) -> Dict[str, Any]:
        ...
        microkernel_count = sum(
            ...
            if k.avg_time_us < self.MICROKERNEL_THRESHOLD_US
        )
