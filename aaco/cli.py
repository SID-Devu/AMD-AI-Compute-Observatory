...
from aaco.analytics.diff import diff_sessions
    ...
    try:
        ...
        if sys_sampler:
            session.save_artifact(
                "sys_samples.json", [s.__dict__ for s in sys_sampler.get_samples()]
            )
        if gpu_sampler:
            session.save_artifact(
                "gpu_samples.json", [s.__dict__ for s in gpu_sampler.get_samples()]
            )
        ...
        click.echo(
            f"P50/P99:         {derived_metrics.measurement_phase.p50_ms:.3f} / {derived_metrics.measurement_phase.p99_ms:.3f} ms"
        )
        click.echo(
            f"Throughput:      {derived_metrics.throughput.get('inferences_per_sec', 0):.1f} infer/sec"
        )
        click.echo(
...
def sweep(
    ...
    for batch_size in batch_sizes:
        ...
        try:
            ...
            click.echo(
                click.style(
                    f" {mean_latency_ms:>8.2f}ms  {throughput_ips:>8.1f} infer/s",
                    fg="green",
                )
            )
    ...
    if len(batch_points) < 2:
        click.echo(
            click.style(
                "\nInsufficient data for scaling analysis (need >= 2 batch sizes)",
                fg="yellow",
            )
        )
        sys.exit(1)
...
@click.option(
    "--baseline-dir",
    "-b",
    default="./aaco_sessions",
    help="Baseline sessions directory",
)
...
def regression(
    ...
    if report.has_regression:
        ...
        for r in report.top_regressions[:5]:
            ...
            click.echo(
                f"  {r.metric_name:<20} {direction} {r.percent_change:+.1f}% (Z={r.z_score:.2f})"
            )
...
def perfetto(
    ...
    if open_ui:
        ...
        click.echo("\nTo visualize:")
        click.echo("  1. Open https://ui.perfetto.dev")
...
def calibrate(gpu_id: int, output: Optional[str], force: bool) -> None:
    ...
    if not force and out_path.exists():
        ...
        click.echo(
            f"Using cached envelope (age: {(time.time() - envelope.timestamp_utc) / 3600:.1f}h)"
        )
    else:
    ...
    click.echo(
        f"  Read BW:            {envelope.bandwidth.read_bandwidth_gbps:.0f} GB/s ({envelope.bandwidth.read_efficiency:.0%})"
    )
    click.echo(f"  Peak Compute:       {envelope.compute.peak_tflops_fp32:.1f} TFLOPS (FP32)")
    click.echo(
        f"  GEMM Throughput:    {envelope.compute.gemm_tflops_fp32:.1f} TFLOPS ({envelope.compute.fp32_efficiency:.0%})"
    )
...
@click.option(
    "--action",
    type=click.Choice(["stats", "trend", "compare", "sessions"]),
    default="stats",
