...
class DurationDistribution:
    ...
    def similarity(self, other: 'DurationDistribution') -> float:
        ...
        import math
        mean_sim = 1.0 / (1.0 + abs(
            math.log10(max(1, self.mean_ns)) - 
            math.log10(max(1, other.mean_ns))
        ))
...
class GridSignature:
    ...
    def signature_hash(self) -> str:
        """Get signature hash."""
        data = f"{self.grid_x}:{self.grid_y}:{self.grid_z}:" \
...
class KernelFamilyClassifier:
    ...
    FAMILY_PATTERNS = {
        ...
        KernelFamily.ELEMENTWISE: ['elementwise', 'ewise', 'pointwise', 'add_', 'mul_', 'relu'],
    ...
    def classify_kernel(
        ...
        if durations:
            ...
            if fp.duration_distribution.mean_ns > 0:
                fp.cov_duration = (
                    fp.duration_distribution.std_dev_ns / 
                    fp.duration_distribution.mean_ns
                )
    ...
    def _assign_to_family(self, fp: KernelFingerprint) -> str:
        ...
        if best_cluster:
            ...
            best_cluster.total_time_ns += (
                fp.duration_distribution.mean_ns * fp.launch_count
            )
    ...
    def compute_family_drift(
        self,
        baseline_fingerprints: Dict[str, KernelFingerprint]
    ...
    def get_family_summary(self) -> Dict[str, Any]:
        ...
        for cluster in self._family_clusters.values():
            families.append({
                ...
                'time_share_pct': (
                    cluster.total_time_ns / total_time * 100 
