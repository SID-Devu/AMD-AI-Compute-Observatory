    ...
    def _default_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            KernelFamily.GEMM.value: [
                r"\bgemm\b", r"\bmatmul\b", r"matrix_mult", r"_mm_",
                r"\bsgemm\b", r"\bdgemm\b", r"\bhgemm\b",
                r"rocblas.*gemm", r"ck::.*Gemm",
                r"cutlass.*gemm", r"cublas.*gemm",
            ],
            KernelFamily.CONV.value: [
                r"\bconv\b", r"convolution", r"winograd", r"im2col",
                r"miopen.*conv", r"ck::.*Conv", r"cudnn.*conv",
            ],
            KernelFamily.ATTENTION.value: [
                r"attention", r"self_attn", r"cross_attn", r"multi_head",
                r"flash.*attn", r"scaled_dot", r"qkv_", r"_qk_", r"_sv_",
            ],
            KernelFamily.SOFTMAX.value: [
                r"\bsoftmax\b", r"soft_max", r"log_softmax",
            ],
            KernelFamily.NORMALIZATION.value: [
                r"layernorm", r"batchnorm", r"groupnorm", r"rmsnorm",
                r"instancenorm", r"layer_norm", r"batch_norm",
                r"_ln_", r"_bn_",
            ],
            KernelFamily.ELEMENTWISE.value: [
                r"\brelu\b", r"\bgelu\b", r"\bsilu\b", r"\bsigmoid\b",
                r"\btanh\b", r"\bswish\b", r"activation",
                r"\badd\b", r"\bmul\b", r"\bdiv\b", r"\bsub\b",
                r"fused_.*add", r"bias_", r"_bias",
                r"element_?wise", r"point_?wise",
            ],
            KernelFamily.REDUCTION.value: [
                r"\breduce\b", r"\bsum\b", r"\bmean\b", r"\bmax\b", r"\bmin\b",
                r"argmax", r"argmin", r"all_reduce",
            ],
            KernelFamily.COPY.value: [
                r"\bcopy\b", r"memcpy", r"memset", r"_copy_",
                r"HtoD", r"DtoH", r"DtoD", r"async_copy",
            ],
            KernelFamily.TRANSPOSE.value: [
                r"\btranspose\b", r"\bpermute\b", r"_transpose",
            ],
            KernelFamily.EMBEDDING.value: [
                r"\bembed\b", r"embedding", r"lookup", r"gather",
                r"index_select", r"scatter",
            ],
            KernelFamily.INIT.value: [
                r"\binit\b", r"initialize", r"_init_",
            ],
            KernelFamily.FILL.value: [
                r"\bfill\b", r"\bzero\b", r"\bones\b", r"constant",
            ],
        }
...
class FamilyClassifier:
    ...
    def classify(self, kernel_name: str,
                grid_size: int = 0,
                block_size: int = 0,
                duration_ns: int = 0) -> ClassificationResult:
        """
    ...
    def _enhance_with_behavior(self, result: ClassificationResult,
                               duration_ns: int,
                               grid_size: int,
                               block_size: int) -> None:
        ...
        compute_families = {KernelFamily.GEMM, KernelFamily.CONV, KernelFamily.ATTENTION}
        memory_families = {KernelFamily.ELEMENTWISE, KernelFamily.COPY, 
                          KernelFamily.REDUCTION, KernelFamily.EMBEDDING}
