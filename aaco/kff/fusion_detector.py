...
class FusionDetector:
    ...
    FUSED_INDICATORS = [
        r"fused_", r"_fused", r"flash_", r"_flash",
        r"_combined", r"combined_", r"merged_",
    ]
    ...
    def _find_pattern_match(self, trace: List[Dict[str, Any]],
                           pattern: FusionPattern) -> Optional[FusionMatch]:
    ...
    def find_fusion_opportunities(self, kernel_trace: List[Dict[str, Any]]
        ...
        for i in range(len(kernel_trace) - window_size + 1):
            ...
            all_small = all(
                k.get("duration_ns", 0) < 10_000 for k in window
            )
            
            if all_small:
                families = [self._infer_family(k.get("name", "")) for k in window]
                total_time = sum(k.get("duration_ns", 0) for k in window)
...
class FusionAnalyzer:
    ...
    def analyze(self, kernel_trace: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for opp in opportunities:
            kernel_time = sum(
                k.get("duration_ns", 0) for k in kernel_trace
                if k.get("name") in opp.kernels
            )
