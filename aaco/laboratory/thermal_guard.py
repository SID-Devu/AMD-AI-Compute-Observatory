...
class ThermalGuard:
    ...
    def __init__(self, gpu_ids: Optional[List[int]] = None, sample_interval_ms: float = 100.0):
        """Initialize thermal guard."""
        self.gpu_ids = gpu_ids or [0]
        self.sample_interval_ms = sample_interval_ms
        self._monitoring = False
        self._monitor_thread: Optional[threading.Thread] = None
        self._samples: Dict[int, deque] = {gpu_id: deque(maxlen=10000) for gpu_id in self.gpu_ids}
    ...
    def stop_monitoring(self) -> Dict[int, ThermalProfile]:
        ...
        return {gpu_id: self._compute_thermal_profile(gpu_id) for gpu_id in self.gpu_ids}
    ...
    def _compute_cis(self, profile: ThermalProfile, clocks: List[float]) -> float:
        """
        Compute Clock Instability Score (CIS).
        ...
        cis = 0.5 * variance_score + 0.3 * drop_score + 0.2 * range_score
    ...
    def validate_thermal_stability(self, profiles: Dict[int, ThermalProfile]) -> Dict[str, Any]:
        """
