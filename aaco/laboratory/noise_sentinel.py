...
class NoiseSentinel:
    ...
    def start_monitoring(self, interval_ms: float = 10.0) -> None:
        ...
        self._monitor_thread = threading.Thread(
            target=self._monitor_loop, args=(interval_ms,), daemon=True
        )
    ...
    def _compute_noise_profile(self) -> NoiseProfile:
        ...
        if not samples:
            return NoiseProfile(window_start=time.time(), window_end=time.time())
    ...
    def _compute_context_switch_rate(self, samples: List[Dict[str, Any]], duration: float) -> float:
    ...
    def _compute_page_fault_rate(self, samples: List[Dict[str, Any]], duration: float) -> float:
    ...
    def _compute_fpi(self, profile: NoiseProfile) -> float:
        ...
        fpi = 0.6 * fault_norm + 0.4 * major_ratio
    ...
    def _compute_stability_score(self, profile: NoiseProfile) -> float:
        ...
        if profile.scheduler_jitter_us > self.thresholds.max_scheduler_jitter_us:
            penalty = min(
                0.3,
                (profile.scheduler_jitter_us - self.thresholds.max_scheduler_jitter_us) / 1000.0,
            )
        ...
        if profile.context_switch_rate > self.thresholds.max_context_switch_rate:
            penalty = min(
                0.2,
                (profile.context_switch_rate - self.thresholds.max_context_switch_rate) / 10000.0,
            )
            penalties.append(penalty)
        ...
        if profile.other_process_cpu_pct > self.thresholds.max_background_cpu_pct:
            penalty = min(
                0.3,
                (profile.other_process_cpu_pct - self.thresholds.max_background_cpu_pct) / 100.0,
            )
            penalties.append(penalty)
