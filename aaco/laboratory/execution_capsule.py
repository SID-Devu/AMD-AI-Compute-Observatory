...
class CapsuleManifest:
    ...
    def compute_environment_hash(self) -> str:
        ...
        combined = self.hardware.fingerprint_hash() + self.software.fingerprint_hash()
...
class ExecutionCapsule:
    ...
    def _get_rocm_version(self) -> str:
        ...
        try:
            result = subprocess.run(
                ["rocm-smi", "--showversion"], capture_output=True, text=True, timeout=5
            )
            if result.returncode == 0:
    ...
    def detect_environment_drift(self, reference_manifest: CapsuleManifest) -> Dict[str, Any]:
        """
        ...
        # Compare hardware
        if (
            self.manifest.hardware.fingerprint_hash()
        ...
        # Compare software
        if (
            self.manifest.software.fingerprint_hash()
    ...
    def _detect_runtime_drift(self) -> bool:
        ...
        if start.gpu_temps_celsius and end.gpu_temps_celsius:
            temp_delta = max(
                abs(e - s) for s, e in zip(start.gpu_temps_celsius, end.gpu_temps_celsius)
            )
