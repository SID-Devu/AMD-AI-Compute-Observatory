...
class StabilityValidator:
    ...
    def validate(self, measurements: List[float]) -> StabilityReport:
        ...
        report.recommendations = self._generate_recommendations(report, measurements, outliers)
    ...
    def _detect_outliers(self, data: List[float], median: float, mad: float) -> List[int]:
    ...
    def compare_distributions(
        ...
        diff_pct = (
            (current_report.median - baseline_report.median) / baseline_report.median * 100
            if baseline_report.median > 0
            else 0
        )

        # Compute noise-aware threshold
        # Use quadrature addition of MADs
        combined_uncertainty = (baseline_report.mad**2 + current_report.mad**2) ** 0.5
