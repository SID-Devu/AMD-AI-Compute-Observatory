...
class InterferenceSampler:
    ...
    def sample_delta(self) -> Optional[Dict[str, Any]]:
        ...
        # Calculate rates
        rate_keys = [
            "context_switches", "irq_total", "softirq_total",
        ...
        try:
            ...
            for line in content.split('\n'):
                ...
                if key in ["pgfault", "pgmajfault", "pswpin", "pswpout",
                           "pgpgin", "pgpgout", "numa_miss", "numa_foreign",
...
class InterferenceClassifier:
    ...
    def classify_context_switches(self, rate: float) -> Tuple[NoiseClassification, float]:
        ...
        if rate < self.thresholds.context_switch_warning:
            ...
            impact = (rate - self.thresholds.context_switch_warning) / \
                     (self.thresholds.context_switch_severe - self.thresholds.context_switch_warning) * 10
    ...
    def classify_irq_rate(self, rate: float) -> Tuple[NoiseClassification, float]:
        ...
        if rate < self.thresholds.irq_warning:
            ...
            impact = (rate - self.thresholds.irq_warning) / \
                     (self.thresholds.irq_severe - self.thresholds.irq_warning) * 5
...
class NoiseSentinelV2:
    ...
    def _establish_baseline(self, num_samples: int = 5, interval: float = 0.1) -> None:
        ...
        if samples:
            ...
            self._baseline = {
                "context_switches_per_sec": sum(s.get("context_switches_per_sec", 0) for s in samples) / len(samples),
                "irq_total_per_sec": sum(s.get("irq_total_per_sec", 0) for s in samples) / len(samples),
                "softirq_total_per_sec": sum(s.get("softirq_total_per_sec", 0) for s in samples) / len(samples),
                "pgmajfault_per_sec": sum(s.get("pgmajfault_per_sec", 0) for s in samples) / len(samples),
            }
    ...
    def _analyze_sample(self, sample: Dict[str, Any]) -> None:
        ...
        if cs_rate > 0:
            ...
            if classification != NoiseClassification.NEGLIGIBLE:
                self._events.append(NoiseEventV2(
    ...
    def _generate_report(self) -> NoiseReportV2:
        ...
        significant = [e for e in self._events if e.classification in 
                      [NoiseClassification.MODERATE, NoiseClassification.SIGNIFICANT, NoiseClassification.SEVERE]]
        ...
        if self._samples:
            noisy_samples = sum(1 for s in self._samples 
        ...
        # Calculate peaks
        if self._samples:
            report.peak_context_switches_per_sec = max(s.get("context_switches_per_sec", 0) for s in self._samples)
        ...
        score = 0.0
        severe_count = sum(1 for e in self._events if e.classification == NoiseClassification.SEVERE)
        moderate_count = sum(1 for e in self._events if e.classification == NoiseClassification.MODERATE)
        ...
        if report.peak_memory_pressure > self.thresholds.memory_pressure_severe:
            report.recommendations.append("Increase available memory or reduce memory pressure from other processes")
        ...
        if NoiseSourceV2.PAGE_FAULT.name in report.event_counts:
            report.recommendations.append("Ensure working set fits in memory; consider mlocking critical pages")
        
        if not report.is_clean:
            report.recommendations.append("Consider re-running measurement during lower system activity")
