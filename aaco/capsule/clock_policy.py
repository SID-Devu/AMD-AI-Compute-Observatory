...
class ClockPolicy:
    ...
    def performance(cls) -> "ClockPolicy":
        ...
        return cls(
            ...
            gpu_policy=GPUClockPolicy(power_profile=GPUPowerProfile.COMPUTE),
            enforce_cpu=True,
            enforce_gpu=True,
        )
...
class ClockEnforcer:
    ...
    def _get_gpu_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get fan
            result = subprocess.run(
                ["rocm-smi", "--showfan"], capture_output=True, text=True, timeout=5
            )
            if result.returncode == 0:
            ...
            # Get power profile
            result = subprocess.run(
                ["rocm-smi", "--showprofile"], capture_output=True, text=True, timeout=5
            )
            if result.returncode == 0:
                for line in result.stdout.split("\n"):
    ...
    def _discover_gpu_clocks(self) -> None:
        ...
        try:
            result = subprocess.run(
                ["rocm-smi", "--showclkfrq"], capture_output=True, text=True, timeout=5
            )
            if result.returncode == 0:
                for line in result.stdout.split("\n"):
    ...
    def restore(self) -> None:
        ...
        try:
            subprocess.run(["rocm-smi", "--resetclocks"], capture_output=True, timeout=10)
            subprocess.run(["rocm-smi", "--setprofile", "auto"], capture_output=True, timeout=10)
            subprocess.run(["rocm-smi", "--resetfans"], capture_output=True, timeout=10)
        except:
            pass
    ...
    def get_clock_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "duration_ns": self._clock_samples[-1].timestamp_ns
            - self._clock_samples[0].timestamp_ns,
        ...
        if gpu_clocks:
            stats["gpu_clock"] = {
                ...
                "variance_pct": (max(gpu_clocks) - min(gpu_clocks)) / max(gpu_clocks) * 100
                if max(gpu_clocks) > 0
                else 0,
            }
