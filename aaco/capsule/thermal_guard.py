...
class ThermalGuard:
    ...
    def _collect_sample(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Clock
            result = subprocess.run(
                ["rocm-smi", "--showclocks"], capture_output=True, text=True, timeout=2
            )
            if result.returncode == 0:
            ...
            # Power
            result = subprocess.run(
                ["rocm-smi", "--showpower"], capture_output=True, text=True, timeout=2
            )
            if result.returncode == 0:
                for line in result.stdout.split("\n"):
        ...
        if "gpu_power_w" in sample and "gpu_power_cap_w" in sample:
            ...
            if power_pct >= config.gpu_power_violation_pct:
                self._add_violation(
                    ...
                    ViolationSeverity.VIOLATION,
                    power_pct,
                    config.gpu_power_violation_pct,
            ...
            if first_clock > 0:
                ...
                if variance_pct >= config.clock_variance_violation_pct:
                    self._add_violation(
                        ...
                        ViolationSeverity.VIOLATION,
                        variance_pct,
                        config.clock_variance_violation_pct,
    ...
    def _detect_thermal_throttle(self, sample: Dict[str, Any], severity: ViolationSeverity) -> None:
    ...
    def _generate_report(self) -> StabilityReport:
        ...
        if self._samples:
            report.duration_ns = (
                self._samples[-1]["timestamp_ns"] - self._samples[0]["timestamp_ns"]
            )
        ...
        critical_count = sum(
            1 for v in self._violations if v.severity == ViolationSeverity.CRITICAL
        )
        violation_count = sum(
            1 for v in self._violations if v.severity == ViolationSeverity.VIOLATION
        )
        ...
        if report.gpu_temp_max_c > self.config.gpu_temp_warning_c:
            report.recommendations.append(
                "Consider improving GPU cooling or reducing workload intensity"
            )

        if report.gpu_clock_max_mhz > 0 and report.gpu_clock_min_mhz > 0:
            variance = (
                (report.gpu_clock_max_mhz - report.gpu_clock_min_mhz)
                / report.gpu_clock_max_mhz
                * 100
            )
            if variance > 5:
        ...
        if self._throttle_events:
            report.recommendations.append(
                "Thermal throttling detected - allow GPU to cool before next run"
            )
...
class PowerGuard:
    ...
    def add_sample(
        self, power_w: float, power_cap_w: float, clock_mhz: int, temp_c: float
