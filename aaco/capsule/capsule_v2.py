...
class MeasurementCapsuleV2:
    ...
    def _check_thermal_state(self) -> None:
        ...
        try:
            ...
            if match:
                ...
                self._clock_samples.append({"timestamp_ns": time.time_ns(), "gpu_temp_c": temp})
    ...
    def _compute_health_score(self) -> CapsuleHealthScore:
        ...
        if noise_count > 0:
            ...
            penalties.append(
                {"type": "noise_events", "count": noise_count, "penalty": noise_penalty}
            )
        ...
        if self.cgroup and not self.cgroup.active:
            isolation_score -= 20
            penalties.append({"type": "cgroup_inactive", "penalty": 20})
    ...
    def set_workload_signature(
        self, model_hash: str, input_signature: str, execution_provider: str
    ) -> None:
