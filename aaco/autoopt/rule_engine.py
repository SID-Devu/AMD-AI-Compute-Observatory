...
class RuleEngine:
    ...
    def evaluate(
        self, context: Dict[str, Any], categories: Optional[List[RuleCategory]] = None
    ) -> RuleResult:
        """
    ...
    def _match_rule(self, rule: OptimizationRule, context: Dict[str, Any]) -> RuleMatch:
    ...
    def _check_conflict(self, rule1: OptimizationRule, rule2: OptimizationRule) -> Optional[str]:
    ...
    def _order_actions(self, matches: List[RuleMatch]) -> List[tuple]:
        ...
        sorted_matches = sorted(matches, key=lambda m: m.rule.priority.value)
