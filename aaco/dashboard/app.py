...
def create_dashboard():
    ...
    with st.sidebar:
        ...
        # Session selector
        session_options = {
            f"{s['model']} / {s['backend']} ({s['session_id'][:8]})": s['path']
            for s in sessions
        }
...
def render_overview(data: Dict[str, Any]):
    ...
    with col3:
        ...
        st.metric(
            "Throughput",
            f"{throughput.get('inferences_per_sec', 0):.1f} inf/s"
        )
    ...
    with col4:
        ...
        st.metric(
            "GPU Active",
            f"{efficiency.get('gpu_active_ratio', 0)*100:.1f}%"
        )
...
def render_kernels(data: Dict[str, Any]):
    ...
    if not df.empty:
        ...
        st.dataframe(
            df[["kernel_name", "calls", "avg_time_us", "total_time_ms", "pct"]].rename(columns={
                "kernel_name": "Kernel",
                "calls": "Calls",
                "avg_time_us": "Avg (μs)",
...
def render_timeline(data: Dict[str, Any]):
    ...
    if measure_results:
        ...
        fig.add_hline(y=mean_lat, line_dash="dash", line_color="gray",
    ...
    if measure_results:
        ...
        if spikes:
            ...
            for idx, lat in spikes[:5]:
                st.write(f"- Iteration {idx}: {lat:.2f}ms ({(lat/median-1)*100:.0f}% above median)")
        else:
