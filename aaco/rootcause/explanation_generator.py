...
from .cause_ranker import RankedCause, RankingResult
from .evidence_collector import Evidence, EvidenceChain
...
class ExplanationGenerator:
    ...
    EXECUTIVE_CATEGORY_DESC = {
        CauseCategory.HARDWARE_MEMORY_BANDWIDTH: 
            "Data transfer bottleneck limiting processing speed",
        CauseCategory.HARDWARE_COMPUTE_BOUND:
            "Compute resources fully utilized",
        CauseCategory.HARDWARE_CACHE_MISS:
            "Data access inefficiency",
        CauseCategory.HARDWARE_OCCUPANCY:
            "GPU underutilization",
        CauseCategory.SOFTWARE_KERNEL_INEFFICIENCY:
            "Suboptimal kernel implementation",
        CauseCategory.SOFTWARE_FUSION_MISSED:
            "Missed optimization opportunity",
        CauseCategory.CONFIG_BATCH_SIZE:
            "Suboptimal batch size configuration",
        CauseCategory.CONFIG_PRECISION:
            "Precision/accuracy trade-off opportunity",
        CauseCategory.MODEL_ARCHITECTURE:
    ...
    ENGINEER_CATEGORY_DESC = {
        CauseCategory.HARDWARE_MEMORY_BANDWIDTH:
            "Memory-bound: HBM bandwidth saturated while compute units idle",
        CauseCategory.HARDWARE_COMPUTE_BOUND:
            "Compute-bound: VALU/MFMA utilization high, memory bandwidth available",
        CauseCategory.HARDWARE_CACHE_MISS:
            "Cache thrashing: High L1/L2 miss rates degrading memory latency",
        CauseCategory.HARDWARE_OCCUPANCY:
            "Low occupancy: Insufficient waves per CU due to resource constraints",
        CauseCategory.SOFTWARE_KERNEL_INEFFICIENCY:
            "Kernel inefficiency: Suboptimal instruction mix or vectorization",
        CauseCategory.SOFTWARE_FUSION_MISSED:
            "Fusion opportunity: Multiple small kernels could be combined",
        CauseCategory.CONFIG_BATCH_SIZE:
            "Batch size: Small batch underutilizing parallel resources",
        CauseCategory.CONFIG_PRECISION:
            "Precision: FP32 used where FP16/BF16 would suffice",
    }
    ...
    def generate_summary(self,
                         ...
                         level: ExplanationLevel = ExplanationLevel.ENGINEER) -> Explanation:
        """
        ...
        for ranked in ranking_result.ranked_causes[:5]:
            if ranked.recommended_action:
                explanation.recommendations.append(
                    f"[P{ranked.rank}] {ranked.recommended_action}"
                )
    ...
    def _generate_deep_dive(self, cause: RootCause, 
                            explanation: Explanation,
                            evidence_chain: Optional[EvidenceChain]) -> None:
    ...
    def _generate_debug(self, cause: RootCause,
                        explanation: Explanation,
                        evidence_chain: Optional[EvidenceChain]) -> None:
    ...
    def _narrate_evidence(self, chain: EvidenceChain) -> str:
        ...
        for i, evidence in enumerate(chain.evidence_list, 1):
            ...
            if evidence.threshold_exceeded:
                narrative += f"   ⚠️ Threshold exceeded\n"
