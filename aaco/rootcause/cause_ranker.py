...
class CauseRanker:
    ...
    def rank(self, causes: List[RootCause]) -> RankingResult:
        ...
        result.top_actions = [rc.recommended_action for rc in ranked[:5] if rc.recommended_action]

        # Sum addressable impact
        result.addressable_impact_pct = sum(
            rc.cause.impact_pct * rc.actionability_score for rc in ranked
        )
    ...
    def _score_cause(self, cause: RootCause) -> RankedCause:
        ...
        # Actionability score
        ranked.actionability_score = self.CATEGORY_ACTIONABILITY.get(cause.category, 0.5)
    ...
    def register_scorer(
        self, criteria: RankingCriteria, scorer: Callable[[RootCause], float]
    ) -> None:
    ...
    def filter_actionable(
        self, result: RankingResult, min_actionability: float = 0.5
    ) -> List[RankedCause]:
        """Filter to only actionable causes."""
        return [rc for rc in result.ranked_causes if rc.actionability_score >= min_actionability]
