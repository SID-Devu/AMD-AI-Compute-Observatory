...
def _IOC(direction, type_char, nr, size):
    ...
    return (direction << IOC_DIRSHIFT) | (type_char << IOC_TYPESHIFT) | \
...
class AACODriverInterface:
    ...
    def get_stats(self, session_id: int) -> AACOStats:
        ...
        cmd = struct.pack(self.SESSION_CMD_FORMAT, session_id, 0, 0, 
                          id(stats_buf))  # This won't work as-is, simplified
    ...
    def emit_marker(self, session_id: int, marker_id: int, 
                    marker_value: int = 0) -> None:
    ...
    def read_events(self, max_events: int = 100) -> List[AACOEvent]:
        ...
        for i in range(0, len(data), self.EVENT_SIZE):
            ...
            # Unpack event
            (timestamp_ns, session_id, pid, event_type, cpu,
             value1, value2, comm_raw) = struct.unpack(
                "<QIIHH QQ 16s", chunk
            )
            
            comm = comm_raw.rstrip(b'\x00').decode('utf-8', errors='replace')
            
            events.append(AACOEvent(
                timestamp_ns=timestamp_ns,
                session_id=session_id,
                pid=pid,
                event_type=AACOEventType(event_type) if event_type in AACOEventType._value2member_map_ else event_type,
                cpu=cpu,
                value1=value1,
                value2=value2,
                comm=comm,
    ...
    def start_event_reader(self) -> None:
        ...
        self._event_thread = threading.Thread(target=self._event_reader_loop,
                                               daemon=True)
