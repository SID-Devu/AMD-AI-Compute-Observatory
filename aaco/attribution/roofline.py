...
class RooflineModel:
    ...
    def get_ridge_point(
        self, compute_ceiling: str = "Peak FP32", memory_ceiling: str = "HBM Bandwidth"
    ) -> Tuple[float, float]:
        """
    ...
    def get_ceiling_at(self, arithmetic_intensity: float, compute_type: str = "Peak FP32") -> float:
    ...
    def analyze_point(
        self, ai: float, achieved_gflops: float, name: str = "", kernel_name: str = ""
    ) -> RooflinePoint:
        ...
        point.efficiency_pct = (
            (achieved_gflops / effective_ceiling * 100) if effective_ceiling > 0 else 0
        )
...
class RooflineAnalyzer:
    ...
    def analyze_kernel(self, kernel_name: str, counters: CounterReading) -> RooflinePoint:
        """
        ...
        # Analyze point
        point = self.model.analyze_point(
            ai, achieved_gflops, name=kernel_name, kernel_name=kernel_name
        )
    ...
    def analyze_batch(self, kernel_data: List[Tuple[str, CounterReading]]) -> List[RooflinePoint]:
    ...
    def suggest_optimizations(self) -> Dict[str, List[str]]:
        ...
        for point in self.points:
            ...
            if point.arithmetic_intensity < 1.0:
                kernel_suggestions.append("Very low arithmetic intensity - consider kernel fusion")
