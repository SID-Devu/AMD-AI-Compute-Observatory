...
class AttributionEngine:
    ...
    def attribute(
        ...
        for kernel_name, duration_ns in kernel_timings:
            ...
            if layers:
                ...
                if self.method == AttributionMethod.PROPORTIONAL:
                    contributions = self._proportional_attribution(kernel_name, layers, duration_ns)
                else:
        ...
        for layer_name, total_time in layer_times.items():
            ...
            layer_attr.primary_bottleneck = self._aggregate_bottleneck(layer_name, result.kernels)
    ...
    def _proportional_attribution(
        self, kernel_name: str, layers: List[str], duration_ns: int
    ) -> Dict[str, float]:
    ...
    def _aggregate_bottleneck(
        self, layer_name: str, kernels: List[KernelAttribution]
    ) -> BottleneckType:
    ...
    def _compute_overall_efficiency(self, result: AttributionResult) -> float:
        ...
        total_weighted = sum(k.efficiency_pct * k.duration_ns for k in result.kernels)
            ...
            "top_layers": [
                {
                    ...
                    "pct": layer.total_time_ns / result.total_time_ns * 100
            ...
            "top_kernels": [
                {
                    ...
                    "pct": kernel.duration_ns / result.total_time_ns * 100
                    if result.total_time_ns > 0
                    else 0,
