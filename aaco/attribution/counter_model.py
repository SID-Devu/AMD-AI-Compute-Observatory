...
class CounterBasedModel:
    ...
    def _calculate_peak(self):
        ...
        self.peak_valu_tflops = (
            ...
            self.params["gpu_clock_mhz"] * 1e-6
        )
        ...
        self.peak_memory_gbps = (
            ...
            self.params["memory_bus_width_bits"] / 8 * 
            2 / 1000  # DDR factor
        )
    ...
    def analyze(self, counters: CounterReading) -> Dict[str, Any]:
        ...
        utilization = ResourceUtilization(
            ...
            mfma_utilization=min(100, achieved_mfma_tflops / self.peak_mfma_tflops * 100) if self.peak_mfma_tflops > 0 else 0,
            memory_bandwidth_utilization=min(100, achieved_bandwidth_gbps / self.peak_memory_gbps * 100),
            ...
            active_waves_per_cu=counters.waves_launched / self.params["num_cus"] if self.params["num_cus"] > 0 else 0,
        )
    ...
    def _identify_bottleneck(self, counters: CounterReading, 
                             utilization: ResourceUtilization) -> BottleneckType:
    ...
    def _compute_time_breakdown(self, counters: CounterReading, 
    ...
    def _compute_efficiency(self, utilization: ResourceUtilization,
        ...
        if bottleneck == BottleneckType.COMPUTE:
            ...
            return (
                utilization.valu_utilization + 
                utilization.memory_bandwidth_utilization
            ) / 2
