...
class TraceMerger:
    ...
    def add_rocprof_trace(self, events: List[Dict[str, Any]], source_id: str = "rocprof") -> None:
    ...
    def add_rocm_smi_samples(
        self, samples: List[Dict[str, Any]], source_id: str = "rocm_smi"
    ) -> None:
    ...
    def add_capsule_events(self, events: List[Dict[str, Any]], source_id: str = "capsule") -> None:
    ...
    def add_noise_events(self, events: List[Dict[str, Any]], source_id: str = "noise") -> None:
    ...
    def add_perfetto_trace(self, trace_data: Dict[str, Any], source_id: str = "perfetto") -> None:
    ...
    def merge(self) -> MergeResult:
        ...
        if not self._sources:
            return MergeResult(events=[], sources=[], time_range=(0, 0), event_count=0)
        ...
        if self.options.detect_clock_skew:
            ...
            if clock_skew_detected:
                warnings.append(f"Clock skew detected: {clock_skew_ns / 1000:.1f} µs")
        ...
        if self.options.reference_source and self.options.reference_source in self._sources:
            ...
            min_time = min(s.start_time_ns for s in self._sources.values() if s.start_time_ns > 0)
