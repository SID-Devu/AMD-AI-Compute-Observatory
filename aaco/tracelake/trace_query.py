...
class TraceQuery:
    ...
    def query(self, filter: Optional[TraceFilter] = None,
    ...
    def query_kernels(self, name_pattern: Optional[str] = None,
                     min_duration_ns: int = 0) -> List[Dict[str, Any]]:
    ...
    def aggregate_duration(self, filter: Optional[TraceFilter] = None,
                          aggregation: AggregationType = AggregationType.MEAN,
                          percentile: float = 50.0) -> AggregationResult:
    ...
    def group_by_name(self, filter: Optional[TraceFilter] = None) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def summarize_kernels(self, filter: Optional[TraceFilter] = None) -> List[Dict[str, Any]]:
        ...
        for name, events in groups.items():
            ...
            summary = {
                ...
                "p99_ns": sorted_dur[int(len(sorted_dur) * 0.99)] if len(sorted_dur) >= 100 else sorted_dur[-1],
            }
...
class TraceAnalyzer:
    ...
    def identify_microkernels(self, threshold_us: float = 10.0) -> List[Dict[str, Any]]:
        ...
        return [k for k in self.query.summarize_kernels() 
                if k["mean_ns"] < threshold_ns]
