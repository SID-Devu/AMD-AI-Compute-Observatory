...
from .sla_engine import SLAEngine, SLAMetricType
...
class CIIntegration:
    ...
    def _run_regression_gate(
        self, metrics: Dict[str, Any], baseline_values: Dict[str, List[float]]
    ) -> GateResult:
        ...
        for metric_name, current_value in metrics.items():
            if metric_name in baseline_values:
                ...
                current = (
                    [current_value] if isinstance(current_value, (int, float)) else current_value
                )

                reg_result = self.regression_detector.detect(metric_name, baseline, current)
    ...
    def _run_baseline_gate(self, metrics: Dict[str, Any]) -> GateResult:
        ...
        for metric_name, current_value in metrics.items():
            ...
            if baseline_metric:
                delta_pct = (
                    ((current_value - baseline_metric.mean) / baseline_metric.mean * 100)
    ...
    def _write_junit(self, result: CIPipelineResult) -> None:
        ...
        for gate in result.gates:
            ...
            xml_lines.append(
                f'  <testcase name="{gate.gate_name}" time="{gate.evaluation_time_ms / 1000:.3f}">'
            )
            if status:
