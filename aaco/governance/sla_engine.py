...
class SLAEngine:
    ...
    def evaluate(
        ...
        for policy in policies_to_check:
            ...
            if (
                policy.applies_to_models
                and context.get("model_name") not in policy.applies_to_models
            ):
                continue
            ...
            for metric_type, threshold in policy.thresholds.items():
                ...
                # Create check
                check = self._check_metric(metric_type, actual, threshold, policy.warning_pct)
                ...
                if not check.passed:
                    violation = SLAViolation(
                        ...
                        threshold=threshold,
                        actual_value=actual,
                        overage_pct=((actual - threshold) / threshold * 100)
                        if threshold > 0
                        else 0,
