...
class ForensicSchedulerModel:
    ...
    def start_collection(self) -> bool:
        ...
        if self._ebpf_available:
            self._collection_thread = threading.Thread(
                target=self._ebpf_collection_loop, daemon=True
            )
            ...
            self._collection_thread = threading.Thread(
                target=self._proc_collection_loop, daemon=True
            )
            self._collection_thread.start()
        ...
        while self._collecting:
            ...
            if dt > 0:
                ...
                with self._lock:
                    ...
                    minor_delta = current_stats.get("minor_faults", 0) - prev_stats.get(
                        "minor_faults", 0
                    )
                    major_delta = current_stats.get("major_faults", 0) - prev_stats.get(
                        "major_faults", 0
                    )

                    for _ in range(min(minor_delta, 100)):
                        self._fault_samples.append(
                            PageFaultSample(
                                timestamp_ns=time.time_ns(),
                                pid=self.target_pid,
                                address=0,
    ...
    def _compute_sii(self, forensics: SchedulerForensics, duration_s: float) -> float:
        ...
        sii = 0.4 * ctx_norm + 0.35 * rq_norm + 0.25 * wu_norm
    ...
    def _compute_fpi(self, forensics: SchedulerForensics, duration_s: float) -> float:
        ...
        major_ratio = forensics.major_fault_count / total_faults if total_faults > 0 else 0

        # Normalize fault P99 latency (1ms = 1.0)
        latency_norm = min(1.0, forensics.fault_latency.p99 / 1000.0)

        # Weighted combination
        fpi = 0.4 * rate_norm + 0.4 * major_ratio + 0.2 * latency_norm
