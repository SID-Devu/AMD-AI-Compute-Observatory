    ...
    def compare(
        ...
        if bundle_a.metrics.memory_peak_mb > 0:
            result.memory_delta_pct = (
                ...
                / bundle_a.metrics.memory_peak_mb * 100
            )
        ...
        if result.latency_delta_pct > self.regression_threshold_pct:
            result.regressions.append(
                f"Latency increased by {result.latency_delta_pct:.1f}%"
            )
        elif result.latency_delta_pct < -self.regression_threshold_pct:
            result.improvements.append(
                f"Latency decreased by {abs(result.latency_delta_pct):.1f}%"
            )
                ...
                for metric, delta in changes.items():
                    if delta > 20:  # 20% regression
                        causes.append({
                            ...
                            "confidence": 0.85,
                            "evidence": {"kernel": kernel, "metric": metric, "delta": delta},
                        })
    ...
    def _generate_recommendations(
        ...
        for cause in root_causes:
            ...
            if category == "memory":
                ...
                recommendations.append(
                    "Investigate recent changes in code, dependencies, "
                    "or system configuration"
                )
            elif category == "kernel_regression":
                kernel = cause.get("evidence", {}).get("kernel", "unknown")
                recommendations.append(
                    f"Profile kernel {kernel} in detail to identify "
                    "the source of regression"
                )
        ...
        for factor in env_factors:
            ...
            if "Multi-GPU" in factor:
                recommendations.append(
                    "Profile inter-GPU communication to check for bottlenecks"
                )
