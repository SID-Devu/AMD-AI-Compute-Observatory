...
class AutoExperimentEngine:
    ...
    def run_experiment(self, config: ExperimentConfig) -> ExperimentResult:
        ...
        try:
            ...
            baseline_result = self._run_variation(
                "baseline", config.baseline_params, config
            )
            ...
            if result.variation_results:
                best = min(result.variation_results, 
        ...
        if result.best_variation_id:
            logger.info(f"Best variation: {result.best_variation_id} "
        ...
        if result.variation_results:
            ...
            if hypothesis.expected_effect == "improvement":
                ...
                result.hypothesis_confirmed = (
                    var.is_significant and var.delta_pct < 0
                )
    ...
    def run_parameter_sweep(self,
                           param_name: str,
                           values: List[Any],
                           baseline_params: Dict[str, Any]) -> ExperimentResult:
        """
    ...
    def _run_variation(self,
                       variation_id: str,
                       params: Dict[str, Any],
                       config: ExperimentConfig) -> VariationResult:
    ...
    def _compare_to_baseline(self,
        ...
        pooled_std = np.sqrt(
            (baseline.std**2 + variation.std**2) / 2
        )
        if pooled_std > 0:
...
class ExperimentDesigner:
    ...
    def design_experiments_for_root_cause(self, 
                                          root_cause_id: str,
                                          current_params: Dict[str, Any]) -> List[ExperimentConfig]:
        """
    ...
    def _instantiate_template(self,
                              ...
                              current_params: Dict[str, Any]) -> Optional[ExperimentConfig]:
...
def quick_ab_test(runner: ExperimentRunner,
                  baseline_params: Dict[str, Any],
                  variation_params: Dict[str, Any],
                  n_samples: int = 30) -> ExperimentResult:
    """
