...
class PassPipelineAnalyzer:
    ...
    def find_ineffective_passes(self, pipeline: PassPipeline, 
                                 min_runs: int = 3) -> List[str]:
    ...
    def find_expensive_passes(self, pipeline: PassPipeline,
    ...
    def analyze_for_gpu(self, pipeline: PassPipeline) -> Dict[str, Any]:
        ...
        kernel_outlined = any(
            r.pass_name == "gpu-kernel-outlining" and r.changed 
            for r in gpu_results
        )
        
        parallel_mapped = any(
            r.pass_name == "gpu-map-parallel-loops" and r.changed
            for r in gpu_results
        )
    ...
    def _gpu_recommendations(self, kernel_outlined: bool, 
                             parallel_mapped: bool) -> List[str]:
