...
class GraphKernelMapper:
    ...
    def map_by_debug_info(
        self,
        graph_nodes: List[Dict[str, Any]],
        kernel_debug_info: Dict[str, Tuple[str, int]],
    ) -> GraphKernelMapping:
        """
        ...
        mapping.unmapped_kernels = set(kernel_debug_info.keys()) - set(
            mapping.kernel_to_nodes.keys()
        )
    ...
    def map_by_name(
        self, graph_nodes: List[Dict[str, Any]], kernel_names: List[str]
    ) -> GraphKernelMapping:
        """
    ...
    def compute_attribution(
        self, mapping: GraphKernelMapping, kernel_timings: Dict[str, float]
    ) -> Dict[str, float]:
        """
    ...
    def get_mapping_report(self, mapping: GraphKernelMapping) -> Dict[str, Any]:
        ...
        return {
            ...
            "multi_kernel_nodes": sum(
                1 for kernels in mapping.node_to_kernels.values() if len(kernels) > 1
            ),
            "multi_node_kernels": sum(
                1 for nodes in mapping.kernel_to_nodes.values() if len(nodes) > 1
            ),
        }
