...
class MLIRReader(CompilerIRReader):
    ...
    OP_PATTERN = re.compile(r'(%\w+)\s*=\s*"?(\w+(?:\.\w+)?)"?\s*(?:\(([^)]*)\))?\s*(?::)?([^}\n]*)')
    ...
    def _parse_blocks(self, body: str) -> List[IRBlock]:
        ...
        while i < len(block_splits):
            if i == 0:
                ...
                block_args = block_splits[i + 1] if i + 1 < len(block_splits) else ""
        ...
        for line in block_body.split('\n'):
            ...
            if match:
                ...
                inst.is_memory_op = any(
                    mem_op in opcode.lower() 
                    for mem_op in ['load', 'store', 'memref', 'alloc']
                )
                ...
                inst.is_control_flow = any(
                    cf_op in opcode.lower()
                    for cf_op in ['br', 'cond_br', 'return', 'call']
                )
    ...
    def analyze_kernel(self, func: IRFunction) -> Dict[str, Any]:
        ...
        memory_ops = sum(
            count for op, count in inst_counts.items()
