...
class FeatureExtractor:
    ...
    def extract_session_features(
        ...
        measure_latencies = latencies_ms[warmup_count:] if len(latencies_ms) > warmup_count else latencies_ms
        ...
        if features.latency_std_ms > 0:
            spike_threshold = features.latency_mean_ms + self.spike_threshold_std * features.latency_std_ms
    ...
    def _extract_gpu_features(self, features: SessionFeatures, 
        ...
        if clocks:
            ...
            if features.clock_mean_mhz > 0:
                features.clock_stability_score = 1.0 - min(1.0, features.clock_std_mhz / features.clock_mean_mhz)
    ...
    def _extract_kernel_features(self, features: SessionFeatures,
                                  kernels: List[Dict],
    ...
    def _extract_sched_features(self, features: SessionFeatures,
                                 sched_data: Dict,
                                 iter_count: int) -> None:
    ...
    def _compute_chi(self, features: SessionFeatures) -> float:
        ...
        thermal = 1.0 - min(1.0, max(0, features.temp_max_c - 60) / 40.0) if features.temp_max_c > 0 else 1.0
    ...
    def extract_iteration_features(
        ...
        gpu_idx = 0
        samples_per_iter = len(gpu_samples) / len(latencies_ms) if gpu_samples and latencies_ms else 0
...
class FeatureStore:
    ...
    def get_baseline(self, model_name: str, 
                     n_sessions: int = 5) -> Optional[SessionFeatures]:
        """
        ...
        for field_name in [
            "latency_mean_ms", "latency_std_ms", "latency_p50_ms", "latency_p90_ms",
            "latency_p95_ms", "latency_p99_ms", "throughput_its", "cv_pct",
            "spike_ratio", "kar", "launch_tax_score", "power_mean_w", "temp_mean_c",
            "clock_mean_mhz", "chi_score",
        ]:
    ...
    def save(self) -> None:
        ...
        logger.info(f"Feature store saved: {len(self._session_features)} sessions, "
        ...
        for i in range(table.num_rows):
            ...
            for field in asdict(SessionFeatures(session_id="")).keys():
                if field not in row_dict:
                    row_dict[field] = "" if field in ("session_id", "model_name", "backend", "device") else 0
