...
class PerfettoTraceBuilder:
    ...
    def _init_tracks(self) -> None:
        ...
        tracks = [
            PerfettoMetadata(
                AACOTrack.PID_INFERENCE, AACOTrack.TID_LATENCY, "Inference Latency", 0
            ),
            PerfettoMetadata(AACOTrack.PID_INFERENCE, AACOTrack.TID_ITERATIONS, "Iterations", 1),
            PerfettoMetadata(
                AACOTrack.PID_GPU_KERNELS,
                AACOTrack.TID_KERNEL_TIMELINE,
                "GPU Kernels",
                2,
            ),
            PerfettoMetadata(AACOTrack.PID_GPU_METRICS, AACOTrack.TID_POWER, "GPU Power (W)", 3),
            PerfettoMetadata(AACOTrack.PID_GPU_METRICS, AACOTrack.TID_CLOCKS, "GPU Clock (MHz)", 4),
            PerfettoMetadata(AACOTrack.PID_GPU_METRICS, AACOTrack.TID_TEMP, "GPU Temp (°C)", 5),
            PerfettoMetadata(
                AACOTrack.PID_CPU_SCHED,
                AACOTrack.TID_CTX_SWITCHES,
                "Context Switches",
                6,
            ),
            PerfettoMetadata(AACOTrack.PID_DRIVER, AACOTrack.TID_DRIVER_EVENTS, "Driver Events", 7),
        ]
    ...
    def add_gpu_kernel(self, kernel_name: str, start_ns: int, duration_ns: int, **kwargs) -> None:
        ...
        self._events.append(
            PerfettoEvent(
                ...
                args={"full_name": kernel_name, "duration_us": dur_us, **kwargs},
                cname="thread_state_running" if dur_us > 10 else "terrible",  # Color by duration
            ).to_dict()
        )

    def add_gpu_counter(
        self, name: str, time_ns: int, value: float, track_id: int = AACOTrack.TID_POWER
    ) -> None:
    ...
    def add_sched_event(self, event_type: str, time_ns: int, value: int = 0, **kwargs) -> None:
    ...
    def add_driver_event(
        self, event_type: str, time_ns: int, value1: int, value2: int, comm: str = ""
    ) -> None:
    ...
    def add_phase_marker(self, phase_name: str, time_ns: int, phase_type: str = "begin") -> None:
    ...
    def add_latency_spike(self, time_ns: int, latency_ms: float, cause: str = "unknown") -> None:
...
class AACOSessionToPerfetto:
    ...
    def _load_gpu_samples(self) -> None:
        ...
        for sample in samples:
            ...
            if "power_w" in sample:
                self.builder.add_gpu_counter(
                    "power_w", t_ns, sample["power_w"], AACOTrack.TID_POWER
                )

            # Clock
            if "gfx_clock_mhz" in sample:
                self.builder.add_gpu_counter(
                    "gfx_clock_mhz", t_ns, sample["gfx_clock_mhz"], AACOTrack.TID_CLOCKS
                )

            # Temperature
            if "temp_c" in sample:
                self.builder.add_gpu_counter("temp_c", t_ns, sample["temp_c"], AACOTrack.TID_TEMP)
    ...
    def save(self, output_path: Optional[Path] = None, compress: bool = False) -> Path:
