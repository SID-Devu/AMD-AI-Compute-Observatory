...
class CorrelationWindow:
    ...
    def get_summary(self) -> Dict[str, Any]:
        kernel_time = sum(e.duration_ns or 0 for e in self.kernel_events) / 1e6  # ms
...
class TimelineCorrelator:
    ...
    def add_kernel_execution(self, kernel_name: str, t_start_ns: int, t_end_ns: int) -> None:
    ...
    def detect_latency_spikes(self, threshold_pct: float = 50.0) -> List[Dict[str, Any]]:
        """
    ...
    def _analyze_spike(
        ...
        typical_kernel_counts = [
            len(w.kernel_events) for w in all_windows if w.duration_ms < median_latency * 1.2
        ]
        ...
        # Check CPU jitter
        cpu_samples = [e.payload.get("cpu_pct", 0) for e in spike_window.system_events]
        ...
        ctx_samples = [e.payload.get("ctx_switches", 0) for e in spike_window.system_events]
        if ctx_samples and np.std(ctx_samples) > np.mean(ctx_samples):
            evidence["likely_causes"].append(
                {
                    "cause": "scheduling_jitter",
                    "details": "High context switch variance",
                }
            )

        # Check clock drops
        clock_samples = [e.payload.get("gfx_clock_mhz", 0) for e in spike_window.gpu_events]
        if clock_samples:
    ...
    def compute_gpu_active_timeline(self, resolution_ms: float = 10.0) -> Dict[str, List[float]]:
        """
