...
class RegressionGuard:
    ...
    def get_baseline(self, model_name: str) -> Optional[BaselineModel]:
        ...
        baseline_sessions = sorted(sessions, key=lambda s: s.timestamp_utc, reverse=True)[
            1:11
        ]  # Up to 10 sessions, excluding most recent
    ...
    def check_regression(
        ...
        if baseline is None or baseline.session_count < self.min_baseline_sessions:
            report.summary = (
                f"Insufficient baseline data (need {self.min_baseline_sessions} sessions)"
            )
            return report
    ...
    def _analyze_metric(
        ...
        result.threshold_used = (
            self.THRESHOLDS.get(severity, 0) if severity != RegressionSeverity.NONE else 0
        )
        ...
        if result.is_regression:
            ...
            mad_confidence = (
                min(1.0, abs(result.mad_score) / 4.0) if result.mad_score != 0 else z_confidence
            )
    ...
    def _generate_recommendations(self, report: RegressionReport) -> List[str]:
        ...
        for r in report.top_regressions[:3]:
            ...
            if "latency" in metric:
                ...
                recommendations.append(
                    "Temperature increase: Check cooling system, ensure adequate ventilation"
                )

            elif "clock" in metric:
                recommendations.append(
                    "Clock instability: Check power delivery and thermal conditions"
                )
...
def check_regression_threshold(
    ...
    if actual_idx <= max_idx:
        return (
            True,
            f"PASS: Regression severity ({report.overall_severity.value}) within threshold",
        )
    else:
        return (
            False,
            f"FAIL: Regression severity ({report.overall_severity.value}) exceeds threshold ({max_severity.value})",
        )
