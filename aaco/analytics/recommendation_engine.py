...
class BottleneckDetector:
    ...
    def detect(self, features: SessionFeatures) -> Dict[BottleneckClass, float]:
        ...
        if features.kar > 0:
            ...
            if features.launch_tax_score > self.THRESHOLDS["launch_tax_high"]:
                scores[BottleneckClass.LAUNCH_OVERHEAD] = max(
                    scores[BottleneckClass.LAUNCH_OVERHEAD], features.launch_tax_score
                )
        ...
        if features.ctx_switches_per_iter >= self.THRESHOLDS["ctx_switches_high"]:
            scores[BottleneckClass.OS_INTERFERENCE] = min(
                1.0,
        ...
        if features.page_faults_per_iter >= self.THRESHOLDS["page_faults_high"]:
            scores[BottleneckClass.OS_INTERFERENCE] = max(
                ...
                min(
                    1.0,
                    features.page_faults_per_iter / (self.THRESHOLDS["page_faults_high"] * 2),
                ),
            )

        # Framework overhead detection (high CV + low KAR)
        if (
            features.cv_pct >= self.THRESHOLDS["cv_warning"]
            and features.kar < self.THRESHOLDS["kar_low"]
        ):
...
class RecommendationEngine:
    ...
    def analyze(
        ...
        iterations: Optional[List[IterationFeatures]] = None,
    ) -> BottleneckAnalysis:
        """
    ...
    def _generate_recommendations(
        ...
        recommendations.sort(key=lambda r: (priority_order.index(r.priority), -r.confidence))

        return recommendations

    def _create_recommendation(
        self, bottleneck: BottleneckClass, features: SessionFeatures, score: float
    ...
    def get_summary(self, analysis: BottleneckAnalysis) -> str:
        ...
        if analysis.secondary_bottlenecks:
            secondary = ", ".join(
                b.value.replace("_", " ") for b in analysis.secondary_bottlenecks[:2]
            )
