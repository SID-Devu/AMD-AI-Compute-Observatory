...
class ProbabilisticAttributionEngine:
    ...
    def build_attribution(self) -> AttributionResult:
        ...
        for op in self._graph_ops:
            node = AttributionNode(
                ...
                attribution_pct=(op["duration_us"] / graph.total_time_us * 100) 
                               if graph.total_time_us > 0 else 0,
            )
            graph.add_node(node)
            ...
            if parent_id:
                edge = AttributionEdge(
                    ...
                    weight=part["duration_us"] / graph.nodes[parent_id].duration_us 
                           if parent_id in graph.nodes else 0,
                    confidence=0.8,
        ...
        for kernel in self._kernels:
            ...
            if evidences:
                ...
                node.evidence = {"type": ev.evidence_type.value, "strength": ev.strength}
            ...
            if node.parent_id:
                edge = AttributionEdge(
                    ...
                    weight=node.duration_us / graph.nodes[node.parent_id].duration_us
                           if node.parent_id in graph.nodes else 0,
    ...
    def _compute_kernel_evidences(self, 
                                  kernel: Dict[str, Any],
                                  graph: AttributionGraph) -> List[Tuple[str, float, Evidence]]:
        ...
        for node_id, node in graph.nodes.items():
            ...
            name_score = self._compute_name_match_score(
                kernel_name, node.name.lower()
            )
            if name_score > 0.3:
    ...
    def _combine_evidences(self, evidences: List[Evidence]) -> float:
        ...
        weighted_sum = sum(
            e.strength * type_weights.get(e.evidence_type, 0.5) 
            for e in evidences
        )
        total_weight = sum(
            type_weights.get(e.evidence_type, 0.5) 
            for e in evidences
        )
    ...
    def _build_result(self, graph: AttributionGraph) -> AttributionResult:
        ...
        # Confidence metrics
        all_confidences = [n.confidence for n in graph.nodes.values() 
                         if n.level == AttributionLevel.KERNEL]
        if all_confidences:
            result.overall_confidence = np.mean(all_confidences)
            result.low_confidence_nodes = [
                n.node_id for n in graph.nodes.values()
                if n.confidence < self.confidence_threshold
            ]
        
        # Attributed vs unattributed
        result.attributed_time_us = sum(
            n.duration_us for n in graph.nodes.values()
