...
class BayesianRootCauseAnalyzer:
    ...
    def analyze(self) -> RootCauseRanking:
        ...
        logger.info(f"Analysis complete. Primary cause: {ranking.primary_cause.description if ranking.primary_cause else 'Unknown'}")
    ...
    def _build_evidence_rules(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        return {
            "memory_bandwidth": [
                {"metric": "memory_bandwidth_utilization", "threshold": 0.8, "direction": "above", "weight": 2.0},
                {"metric": "compute_utilization", "threshold": 0.5, "direction": "below", "weight": 1.5},
                {"metric": "l2_hit_rate", "threshold": 0.5, "direction": "below", "weight": 1.0},
            ],
            "compute_bound": [
                {"metric": "compute_utilization", "threshold": 0.8, "direction": "above", "weight": 2.0},
                {"metric": "memory_bandwidth_utilization", "threshold": 0.5, "direction": "below", "weight": 1.5},
            ],
            "launch_overhead": [
                {"metric": "avg_kernel_duration_us", "threshold": 10, "direction": "below", "weight": 2.0},
                {"metric": "kernel_count", "threshold": 1000, "direction": "above", "weight": 1.5},
                {"metric": "launch_overhead_pct", "threshold": 0.1, "direction": "above", "weight": 2.0},
            ],
            "sync_stalls": [
                {"metric": "sync_time_pct", "threshold": 0.1, "direction": "above", "weight": 2.0},
                {"metric": "stall_rate", "threshold": 0.2, "direction": "above", "weight": 1.5},
            ],
            "pcie_transfer": [
                {"metric": "transfer_time_pct", "threshold": 0.2, "direction": "above", "weight": 2.0},
                {"metric": "transfer_bandwidth_utilization", "threshold": 0.7, "direction": "above", "weight": 1.5},
            ],
            "low_occupancy": [
                {"metric": "occupancy", "threshold": 0.5, "direction": "below", "weight": 2.0},
                {"metric": "active_warps", "threshold": 0.3, "direction": "below", "weight": 1.5},
            ],
            "cache_thrashing": [
                {"metric": "l2_hit_rate", "threshold": 0.3, "direction": "below", "weight": 2.0},
                {"metric": "l1_hit_rate", "threshold": 0.5, "direction": "below", "weight": 1.5},
            ],
            "thermal_throttle": [
                {"metric": "gpu_temperature_c", "threshold": 85, "direction": "above", "weight": 2.0},
                {"metric": "clock_throttle_detected", "threshold": 0.5, "direction": "above", "weight": 2.5},
            ],
            "memory_fragmentation": [
                {"metric": "allocation_failure_rate", "threshold": 0.01, "direction": "above", "weight": 2.0},
                {"metric": "memory_fragmentation_score", "threshold": 0.3, "direction": "above", "weight": 1.5},
            ],
            "resource_contention": [
                {"metric": "gpu_utilization_variance", "threshold": 0.3, "direction": "above", "weight": 1.5},
                {"metric": "other_processes_gpu_pct", "threshold": 0.1, "direction": "above", "weight": 2.0},
            ],
        }
...
def explain_root_cause(ranking: RootCauseRanking) -> str:
    ...
    if len(ranking.suspects) > 1:
        ...
        for suspect in ranking.suspects[1:4]:
            lines.append(f"  {suspect.rank}. {suspect.description} ({suspect.posterior_probability:.1%})")
