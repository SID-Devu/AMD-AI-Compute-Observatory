...
class BatchScalingAnalyzer:
    ...
    def _compute_scaling_efficiency(
        ...
        ideal_throughputs = [
            base_throughput * (b / base_batch) for b in batch_sizes
        ]
    ...
    def _fit_curve(self, x: List[int], y: List[float]) -> Dict[str, float]:
        ...
        try:
            ...
            return {
                f"c{i}": float(c) for i, c in enumerate(coeffs)
            }
        except Exception:
            return {}
    ...
    def _generate_summary(
        ...
        if efficiency >= 0.9:
            ...
            parts.append(f"Moderate batch scaling ({efficiency:.0%} of linear). Consider optimization.")
        else:
    ...
    def plot_scaling_curves(self, output_path: Optional[str] = None):
        ...
        ax3 = axes[1, 0]
        efficiencies = [
            throughputs[i] / (throughputs[0] * batches[i] / batches[0])
        ...
        if vrams and len(vrams) == len(batches):
            ...
            ax4.text(0.5, 0.5, "VRAM data not available", ha="center", va="center", transform=ax4.transAxes)
            ax4.set_title("Memory Scaling")
