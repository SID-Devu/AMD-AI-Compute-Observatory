...
class LaunchTaxAnalyzer:
    ...
    def _compute_metrics(self) -> LaunchTaxMetrics:
        ...
        micro_calls = sum(k.calls for k in self.kernels if k.avg_time_us < self.threshold_us)
        micro_time = sum(k.total_time_ms for k in self.kernels if k.avg_time_us < self.threshold_us)
        ...
        launch_per_inf = (
            total_calls / self.num_inferences if self.num_inferences > 0 else total_calls
        )
    ...
    def _build_duration_histogram(self) -> List[KernelDurationBucket]:
        ...
        for low, high in self.DURATION_BUCKETS:
            ...
            bucket_kernels = [k for k in self.kernels if low <= k.avg_time_us < high]
    ...
    def _find_top_microkernels(self, top_n: int = 10) -> List[Dict[str, Any]]:
        ...
        micro_kernels = [k for k in self.kernels if k.avg_time_us < self.threshold_us]
    ...
    def _generate_recommendations(
        ...
        if not recs:
            recs.append("Kernel launch overhead is well-controlled. No immediate concerns.")
