...
class KernelEvent:
    ...
    def total_threads(self) -> int:
        return self.grid_x * self.grid_y * self.grid_z * self.block_x * self.block_y * self.block_z
...
class KernelFingerprint:
    ...
    def similarity_to(self, other: "KernelFingerprint") -> float:
        ...
        if self.duration_mean_us > 0 and other.duration_mean_us > 0:
            log_diff = abs(np.log10(self.duration_mean_us) - np.log10(other.duration_mean_us))
            score += 0.3 * max(0, 1 - log_diff / 3)  # 3 orders of magnitude = 0

        # Grid similarity
        if self.total_threads_mean > 0 and other.total_threads_mean > 0:
            log_diff = abs(np.log10(self.total_threads_mean) - np.log10(other.total_threads_mean))
...
class KernelFamilyFingerprinter:
    ...
    def __init__(self, min_samples: int = 5, cluster_threshold: float = 0.5):
        """
        Args:
    ...
    def build_registry(self) -> KernelFamilyRegistry:
        ...
        for family_id, member_names in families.items():
            ...
            family_fp = self._merge_fingerprints(
                family_id, [name_fingerprints[n] for n in member_names]
            )
        ...
        logger.info(
            f"Built registry with {len(registry.families)} families "
            f"from {len(name_fingerprints)} unique kernels"
        )

        return registry

    def _create_fingerprint_from_events(
        self, name: str, events: List[KernelEvent]
    ) -> KernelFingerprint:
        ...
        fp.counter_signature = {k: counter_sums[k] / counter_counts[k] for k in counter_sums}
    ...
    def _cluster_fingerprints(
        self, name_fingerprints: Dict[str, KernelFingerprint]
    ) -> Dict[str, List[str]]:
    ...
    def _merge_fingerprints(
        self, family_id: str, fingerprints: List[KernelFingerprint]
    ) -> KernelFingerprint:
        ...
        merged.duration_mean_us = sum(
            fp.duration_mean_us * fp.member_count / total_members for fp in fingerprints
        )
        merged.duration_p50_us = statistics.median([fp.duration_p50_us for fp in fingerprints])
        ...
        merged.total_threads_mean = statistics.mean([fp.total_threads_mean for fp in fingerprints])
...
def detect_fingerprint_drift(
    baseline: KernelFingerprint, current: KernelFingerprint, threshold: float = 0.2
) -> Dict[str, Any]:
    """
    ...
    if baseline.total_threads_mean > 0:
        thread_pct = (
            current.total_threads_mean - baseline.total_threads_mean
        ) / baseline.total_threads_mean
