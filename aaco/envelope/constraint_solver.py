...
from .gpu_model import GPUModel, GPUSpecs
...
class ConfigurationSpace:
    ...
    # Block size options
    block_sizes: List[Tuple[int, int, int]] = field(default_factory=lambda: [
...
class ConstraintSolver:
    ...
    def __init__(self, gpu_model: GPUModel):
        self.gpu = gpu_model
        self.simulator = PerformanceSimulator(SimulationConfig(
            target_gpu=gpu_model.gfx_version
        ))
    ...
    def solve(self,
              ...
              objective: str = "latency") -> OptimalConfig:
        """
        ...
        for config in configs:
            ...
            # Check constraints
            satisfied = all(
                self._check_constraint(c, config)
                for c in constraints
            )
            
            if satisfied:
                score = config.estimated_time_us if objective == "latency" else -config.estimated_time_us
        ...
        if best_config:
            ...
            result.best_throughput = 1e6 / best_config.estimated_time_us if best_config.estimated_time_us > 0 else 0
        else:
            ...
            result.violated_constraints = [
                ...
                if not self._check_constraint(c, result.config)
    ...
    def _enumerate_configs(self, space: ConfigurationSpace,
                          profile: KernelProfile) -> List[Configuration]:
    ...
    def _evaluate_config(self, config: Configuration,
                        base_profile: KernelProfile) -> None:
        ...
        ) * config.batch_size / 32 / 1024 / 1024
    ...
    def _find_min_violation(self, configs: List[Configuration],
                           constraints: List[Constraint]) -> Optional[Configuration]:
    ...
    def _build_pareto_front(self, configs: List[Configuration]) -> List[Configuration]:
        ...
        for config in configs:
            ...
            for other in configs:
                ...
                if (other.estimated_time_us <= config.estimated_time_us and
                    other.estimated_memory_mb <= config.estimated_memory_mb and
                    other.estimated_occupancy >= config.estimated_occupancy):
                    # Check strict improvement on at least one
                    if (other.estimated_time_us < config.estimated_time_us or
                        other.estimated_memory_mb < config.estimated_memory_mb or
                        other.estimated_occupancy > config.estimated_occupancy):
                        dominated = True
                        break
    ...
    def sensitivity_analysis(self,
                            kernel_profile: KernelProfile,
                            parameter: str,
                            values: List[Any]) -> Dict[Any, float]:
        """
