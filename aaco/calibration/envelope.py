...
class HardwareEnvelopeCalibrator:
    ...
    def _calibrate_bandwidth(self) -> None:
        ...
        if has_bw_test:
            try:
                self._run_cmd(f"rocm-bandwidth-test -d {self.gpu_id}")
                # Parse results...
    ...
    def _calibrate_transfer(self) -> None:
        ...
        # Get PCIe info
        try:
            result = self._run_cmd("rocm-smi --showpciebw --json")
            if result:
                json.loads(result)
...
def compare_envelopes(baseline: HardwareEnvelope, current: HardwareEnvelope) -> Dict[str, Any]:
    """
    ...
    if baseline.bandwidth.read_bandwidth_gbps > 0:
        delta = current.bandwidth.read_bandwidth_gbps - baseline.bandwidth.read_bandwidth_gbps
        pct = (delta / baseline.bandwidth.read_bandwidth_gbps) * 100
        comparison["bandwidth_delta_pct"] = pct
        if abs(pct) > 5:
            comparison["changes"].append(f"Bandwidth changed {pct:+.1f}%")

    # Compute comparison
    if baseline.compute.gemm_tflops_fp32 > 0:
        delta = current.compute.gemm_tflops_fp32 - baseline.compute.gemm_tflops_fp32
        pct = (delta / baseline.compute.gemm_tflops_fp32) * 100
        comparison["compute_delta_pct"] = pct
        if abs(pct) > 5:
            comparison["changes"].append(f"Compute throughput changed {pct:+.1f}%")
    ...
    if baseline.launch.launch_p50_us > 0:
        ...
        if abs(pct) > 20:
            comparison["changes"].append(f"Launch overhead changed {pct:+.1f}%")
...
def load_or_calibrate(
    cache_path: Path, gpu_id: int = 0, max_age_hours: float = 24
) -> HardwareEnvelope:
    """
