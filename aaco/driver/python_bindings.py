...
class IRQStats:
    ...
    def from_bytes(cls, data: bytes) -> 'IRQStats':
        ...
        return cls(total_irqs=vals[0], timer_irqs=vals[1], 
                   ipi_irqs=vals[2], other_irqs=vals[3])
...
class TSCReader:
    ...
    def rdtscp() -> Tuple[int, int]:
        ...
        try:
            import time
            return (int(time.perf_counter_ns()), os.sched_getcpu() if hasattr(os, 'sched_getcpu') else 0)
        except:
            return (0, 0)
